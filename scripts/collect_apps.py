...
LEG = "https://itunes.apple.com/kr/rss/{c}/limit=100/genre={g}/json"
LEG_ALL = "https://itunes.apple.com/kr/rss/{c}/limit=100/json"
V2 = "https://rss.applemarketingtools.com/api/v2/kr/apps/top-free/100/apps.json"

FREE, GROSS = "topfreeapplications", "topgrossingapplications"
...
"은행 앱": (6015, FREE, {
    ...
    "신한 SOL": ["신한sol", "sol뱅크", "신한쏠", "신한은행"],
    "우리WON": ["우리won", "won뱅킹", "우리은행"], "하나원큐": ["하나원큐", "하나은행"],
    "NH올원": ["nh올원", "올원뱅크", "nh스마트뱅킹", "농협"],
    "케이뱅크": ["케이뱅크"], "IBK i-ONE": ["i-one", "아이원", "기업은행"],
    "SC제일": ["sc제일"],
}),
"증권 앱": (6015, FREE, {
    "토스증권": ["토스증권"], "키움 영웅문": ["영웅문", "키움"],
    "미래에셋 M-STOCK": ["m-stock", "엠스탁", "미래에셋"],
    "삼성증권 mPOP": ["mpop", "엠팝", "삼성증권"],
    "NH나무": ["나무증권", "nh투자증권", "nh나무"],
    "KB M-able": ["m-able", "마블", "kb증권"],
...
def pull(url, key):
    ...
    if key == "legacy":
        ...
        if isinstance(e, dict):
            e = [e]
        return [x.get("im:name", {}).get("label", "") for x in e]
    return [x.get("name", "") for x in d.get("feed", {}).get("results", [])]

def get_names(chart, genre):
    for url, key in ((LEG.format(c=chart, g=genre), "legacy"),
                     (LEG_ALL.format(c=chart), "legacy"),
                     (V2, "v2")):
        try:
            names = [n for n in pull(url, key) if n]
            if names:
                print(f"  [ok] {url[:70]}... -> {len(names)}")
                return names
...
try:
    with open("data/apps.json", encoding="utf-8") as f:
        for k, v in json.load(f).get("categories", {}).items():
...
for cat, (genre, chart, alias) in CATS.items():
    print(f"[{cat}]")
    names = get_names(chart, genre)
    if not names:
        continue

    found = {}
    for i, real in enumerate(names, 1):
        an = norm(real)
        best, blen = None, 0
    ...
    if len(found) < 3:
        print(f"  skip: {len(found)}개만 매칭 / 차트 상위: {names[:8]}")
        continue
    ...
    out["categories"][cat] = {
        ...
        "basis": "매출 순위" if chart == GROSS else "iOS 무료 앱 순위",
        "targetDate": today,
        "list": lst,
    }
